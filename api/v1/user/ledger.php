...
try {
    $stmt = $pdo->prepare("SELECT ledger_id, total_mmk, display_currency, status, created_at FROM md_treasury_ledgers WHERE cit_id = :id ORDER BY created_at DESC");
    $stmt->execute(['id' => $cit_id]);
    $ledgers = $stmt->fetchAll();

    send_clean_json(200, ["status" => "success", "data" => $ledgers]);
